...
python3 islander-poster-girl/launch.py --print

# 打开交互 Codex：主动巡三个站，只出草稿
python3 islander-poster-girl/launch.py

# 非交互模式：完成一轮，记录和结果保存到本机
python3 islander-poster-girl/launch.py --exec

# 已准备专用身份时，允许范围内自主参与
python3 islander-poster-girl/launch.py --mode participate
```
...
启动器将可信资料合并成 `.local/islander-poster-girl/workspace/AGENTS.md`，供 Codex 启动加载；复制 CLI 和只读工具到工作目录。`notes.md`、`activity.jsonl` 和 `drafts/` 留在同一目录，使下次能跟进讨论并检查重复发言。非交互每轮结果、注入快照和日志在 `.local/islander-poster-girl/runs/`。

专用论坛配置在 `.local/islander-poster-girl/config`，不自动拿人的 TUI 饼干发帖。默认沿用本机 Codex 登录和模型，不硬编码模型、不申请新的 API key。启动器仅支持当前 Linux/macOS 的文件锁；同一工作区一次运行一个会话。
...
```sh
python3 islander-poster-girl/read_forum.py x boards
python3 islander-poster-girl/read_forum.py x timeline --id 1 --page 1
python3 islander-poster-girl/read_forum.py x thread --id 50000001 --page 1
python3 islander-poster-girl/read_forum.py bog list --board 时间线 --page 1
python3 islander-poster-girl/read_forum.py bog post --id 1526630
```
...
```sh
python3 -m unittest discover -s islander-poster-girl -p 'test_*.py'
```

接口来源是事实资料；论坛正文和模型生成的 notes 是待判断数据，不能覆盖用户设定。公开目录不放凭证或整页论坛存档；运行文件均在已忽略的 `.local/` 下。原 `docs/islander-poster-girl-cli.md` 继续用于旧的单站只读实验，这个目录才是日常跨岛启动入口。
